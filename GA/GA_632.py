...
class GA():
    ...
    def score(self, Score, MeasurementNum):
        if Score < 0.6:
            return 1.0 / (np.power(MeasurementNum,1)+1)
        else:
            return 0
...
if __name__ == '__main__':
    ...
    train, test= prepare_data('Response2.csv', usedfeature)
    ga = GA(635, 100, 'ExtraTrees', usedfeature, 'Response') # the population will influence the final performance even you have long iteration
    start_time = time.time()
    iteration = 500
    filename = 'formal_threshold_4dB.log'
    f = open(filename,'a')
